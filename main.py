...
def main(configFile):
  ...
  metricsFilename = os.path.join(config.folderName.metricsFolder, runName)
  plotFilename = os.path.join(config.folderName.plotFolder, runName)
  ...
  for _ in range(iterationsNum):
    for _ in range(config.replayRatio):
      sampleData = core.buffer.sample(config.batchSize, config.dreamer.batchlength)
