...
class Encoder(nn.Module):
  def __init__(self, inputshape, outputsize, activation, config):
    ...
    self.linear = nn.Linear(config.dreamer.encoder.depth*8*(height//config.dreamer.encoder.stride ** 4)*(width//config.dreamer.encoder.stride ** 4), outputsize)
  ...
  def forward(self, x):
    spatial_feat = self.network(x)
    flattened = self.flatten(spatial_feat)
    output = self.activation(self.linear(flattened))
    return output.view(-1, self.outputsize), spatial_feat
